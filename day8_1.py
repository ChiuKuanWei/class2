    ...
    with ExcelWriter(excel_path, engine='openpyxl') as writer:
        for i in range(1, num_classes + 1):
            ...
            scores_df = pd.DataFrame(scores,
                                     ...
                                     index=range(1, 4)) #資料列索引

            # 計算總分和平均
            sum_values = scores_df.sum(axis=1) #sum(axis=1)用來求矩陣每一行的和，sum(axis=0)用來求矩陣每一列的和
            mean_values = scores_df.mean(axis=1) #sum(axis=1)用來求矩陣每一行的和，sum(axis=0)用來求矩陣每一列的和
